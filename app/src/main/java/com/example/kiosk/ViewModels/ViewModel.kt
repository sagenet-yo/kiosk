...
class EmployeeViewModel : ViewModel() {
    ...
    fun searchEmployees(searchTerm: String) {
        ...
        call.enqueue(object : Callback<List<EmployeeSearchResponse>> {
            override fun onResponse(call: Call<List<EmployeeSearchResponse>>, response: Response<List<EmployeeSearchResponse>>) {
                if (response.isSuccessful) {
                    _employees.value = response.body()?.map { responseItem ->
                        Employee(responseItem.id, responseItem.firstName, responseItem.lastName, "",false)
                    } ?: emptyList()
                }
            }
